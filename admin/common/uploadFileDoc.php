...
echo '
	...
	';
		...
		if ($_FILES['userfile']['name']=="")
		{
			echo '
					 <tr>
					  <td class="tdPopUp" height="22">Subir Archivo
					  </td>
					 </tr>
					 <tr>
					  <td>
						<form enctype="multipart/form-data" action="uploadFileDoc.php" method="post">
		...
		else
		{
			...
			$hayRestriccion = ExisteRestriccionPorTipoArchivoDoc;


			If($hayRestriccion == 1 && fileTypeDocUpload!=$tipo_archivo) //Existe restriccion para un tipo de archivo
			{
				   ...
				   <td>
				     <div id="content">';
						if (move_uploaded_file($_FILES['userfile']['tmp_name'], $directorio.$nombre_archivo)){
						   ...
						   echo "<script>window.opener.document.forms[0].NombreDctoRel.value = '".$nombre_archivo."'</script>";
						}else{
